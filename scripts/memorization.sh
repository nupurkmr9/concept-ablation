...
python train.py -t --gpus 0,1 --concept_type memorization --caption_target "VAN GOGH CAFE TERASSE copy" --prompts assets/finetune_prompts/vangoghcafe_mem.txt --name "vangoghcafe" --mem_impath assets/mem_images/vangoghcafe.png --regularization
...
python train.py -t --gpus 0,1 --concept_type memorization --caption_target "Ann Graham Lotz" --prompts assets/finetune_prompts/anne_mem.txt --name "anne" --mem_impath assets/mem_images/anne.png --regularization --base_lr 5e-7
